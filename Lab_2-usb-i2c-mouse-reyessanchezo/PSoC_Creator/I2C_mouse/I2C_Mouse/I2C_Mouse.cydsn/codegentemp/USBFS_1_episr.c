...
CY_ISR(USBFS_1_LPM_ISR)
{
    ...
    ***************************************************************************/
        ...
        while (0u != arbIntrStatus)
        {
            ...
            if (0u != (arbIntrStatus & 0x01u))
            {
                ...
                if (0u != (epStatus & USBFS_1_ARB_EPX_INT_DMA_TERMIN))
                {
                    ...
                    USBFS_1_DmaEpBurstCnt[ep] = USBFS_1_DMA_GET_BURST_CNT(USBFS_1_DmaEpBurstCntBackup[ep]);
                    ...
                    */
                    ...
                    if (USBFS_1_DmaEpBurstCntBackup[ep] > 1u)
                    {
    ...
    /******************************************************************************
    * Function Name: USBFS_1_EPxDmaDone
    ***************************************************************************//**
    *
    * \internal
    ...
    *
    ******************************************************************************/
    CY_INLINE static void USBFS_1_EPxDmaDone(uint8 dmaCh, uint8 ep)
    {
        uint32 nextAddr;

        /* Manage data elements which remain to transfer. */
        if (0u != USBFS_1_DmaEpBurstCnt[ep])
        {
            if(USBFS_1_DmaEpBurstCnt[ep] <= 2u)
            {
                /* Adjust length of last burst. */
                USBFS_1_CyDmaSetNumDataElements(dmaCh,
                                                    ((uint32) USBFS_1_DmaEpLastBurstEl[ep] >> USBFS_1_DMA_DESCR_SHIFT),
                                                    ((uint32) USBFS_1_DmaEpLastBurstEl[ep] &  USBFS_1_DMA_BURST_BYTES_MASK));
            }
            ...
            if (0u != (USBFS_1_EP[ep].addr & USBFS_1_DIR_IN))
            {
                ...
                nextAddr = (uint32) USBFS_1_CyDmaGetSrcAddress(dmaCh, USBFS_1_DMA_DESCR0);
                nextAddr += (2u * USBFS_1_DMA_BYTES_PER_BURST);
                USBFS_1_CyDmaSetSrcAddress(dmaCh, USBFS_1_DMA_DESCR0, (void *) nextAddr);

                /* Change source for descriptor 1. */
                nextAddr += USBFS_1_DMA_BYTES_PER_BURST;
                USBFS_1_CyDmaSetSrcAddress(dmaCh, USBFS_1_DMA_DESCR1, (void *) nextAddr);
            }
            else
            {
                /* Change destination for descriptor 0. */
                nextAddr  = (uint32) USBFS_1_CyDmaGetDstAddress(dmaCh, USBFS_1_DMA_DESCR0);
                nextAddr += (2u * USBFS_1_DMA_BYTES_PER_BURST);
                USBFS_1_CyDmaSetDstAddress(dmaCh, USBFS_1_DMA_DESCR0, (void *) nextAddr);

                /* Change destination for descriptor 1. */
                nextAddr += USBFS_1_DMA_BYTES_PER_BURST;
                USBFS_1_CyDmaSetDstAddress(dmaCh, USBFS_1_DMA_DESCR1, (void *) nextAddr);
            }

            /* Enable DMA to execute transfer as it was disabled because there were no valid descriptor. */
            USBFS_1_CyDmaValidateDescriptor(dmaCh, USBFS_1_DMA_DESCR0);
            
            --USBFS_1_DmaEpBurstCnt[ep];
            if (0u != USBFS_1_DmaEpBurstCnt[ep])
            {
                USBFS_1_CyDmaValidateDescriptor(dmaCh, USBFS_1_DMA_DESCR1);
                --USBFS_1_DmaEpBurstCnt[ep];
            }
            
            USBFS_1_CyDmaChEnable (dmaCh);
            USBFS_1_CyDmaTriggerIn(USBFS_1_DmaReqOut[ep]);
        }
        else
        {
        ...
        ******************************************************************************/
        void USBFS_1_EP1_DMA_DONE_ISR(void)
        {
            ...
            USBFS_1_EPxDmaDone((uint8)USBFS_1_EP1_DMA_CH,
                                                  USBFS_1_EP1);
        ...
        ******************************************************************************/
        void USBFS_1_EP2_DMA_DONE_ISR(void)
        {
            ...
            USBFS_1_EPxDmaDone((uint8)USBFS_1_EP2_DMA_CH,
                                                  USBFS_1_EP2);
        }
        ...
        ******************************************************************************/
        void USBFS_1_EP3_DMA_DONE_ISR(void)
        {
            ...
            USBFS_1_EPxDmaDone((uint8)USBFS_1_EP3_DMA_CH,
                                                  USBFS_1_EP3);
        }
        ...
        ******************************************************************************/
        void USBFS_1_EP4_DMA_DONE_ISR(void)
        {
            ...
            USBFS_1_EPxDmaDone((uint8)USBFS_1_EP4_DMA_CH,
                                                  USBFS_1_EP4);
        }
        ...
        ******************************************************************************/
        void USBFS_1_EP5_DMA_DONE_ISR(void)
        {
            ...
            USBFS_1_EPxDmaDone((uint8)USBFS_1_EP5_DMA_CH,
                                                  USBFS_1_EP5);
        }
        ...
        ******************************************************************************/
        void USBFS_1_EP6_DMA_DONE_ISR(void)
        {
            ...
            USBFS_1_EPxDmaDone((uint8)USBFS_1_EP6_DMA_CH,
                                                  USBFS_1_EP6);
        }
        ...
        ******************************************************************************/
        void USBFS_1_EP7_DMA_DONE_ISR(void)
        {
            ...
            USBFS_1_EPxDmaDone((uint8)USBFS_1_EP7_DMA_CH,
                                                  USBFS_1_EP7);
        }
        ...
        ******************************************************************************/
        void USBFS_1_EP8_DMA_DONE_ISR(void)
        {
            ...
            USBFS_1_EPxDmaDone((uint8)USBFS_1_EP8_DMA_CH,
                                                  USBFS_1_EP8);
        }
